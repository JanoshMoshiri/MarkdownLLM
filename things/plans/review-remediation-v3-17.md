...
- [ ] **1. Phantom `mdllm orient`.** `_dk_session_start` (≈:1908) emits
...
- [ ] **2. MCP egress leak.** Add `informed_by` and `parties` to
...
- [ ] **3. Silent relationship-trigger skip.** `cmd_triggers` evaluates
...
- [ ] **5. `stale` trigger reads mtime, not git.** (Fifth review.) mtime is wrong
...
- [ ] **6. Reverse TIERS check.** Coherence verifies catalog→TIERS only; add
...
- [ ] **4. Kernel token figure, third strike.** Live figure is 2,077 (`mdllm tokens`);
  ...
  `mdllm tokens`.
- [ ] **7. Duplicate birth path.** domain-specification-guide.md Step 1 (≈:627) still
...
- [ ] **8. Stale CONTRIBUTING.** (Fifth review.) Refresh, including its third manual
  birth variant — same single-path rule as item 7.
- [ ] **9. git-workflow "Three Layers" heading** over two-layer text. (Fifth review.)
- [ ] **10. session-memory live-voice continuity-brief reference.** (Fifth review;
  continuity.md is retired — the live voice must not still teach it.)
- [ ] **11. workflow-definition template `instance-of` residue**, plus the surviving
  body instance in examples/life-manager/home-renovation-process.md. (Fifth review +
  sixth's addendum.)
- [ ] **12. Examples pinned at 3.4.0.** Refresh examples to v3.17 shape; add the
