...
is most pulled elsewhere. It is therefore the prime hardening target — delivered
mechanically at t=0 via a `SessionStart` adapter injecting the ritual, not by sharper
prose.
...
- It marks the one honest exception to "interpretation is sufficient for correctness":
  session-start is where the interpretation floor is weakest, which is exactly why the
  adapter is *hardening*, not gold-plating.
