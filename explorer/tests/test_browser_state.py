...
def test_document_surface_state_machine(tmp_path):
    ...
    result = subprocess.run(
        [node, str(harness), str(modules["state"]), str(modules["routing"])],
        capture_output=True,
        text=True,
        timeout=5,
    )
