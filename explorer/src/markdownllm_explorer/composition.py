...
from .adapters.git_commit_history import GitCommitHistory
...
def build_runtime(root: Path, domain_dir: str = "domain", *, limits: ExplorerLimits | None = None) -> ExplorerRuntime:
    ...
    catalogue = FilesystemSourceCatalogue(root, domain_dir, registry, policy)
    cursors = CursorCodec(secrets.token_bytes(32))
    source_browser = ConfinedSourceReader(
        registry, policy, active_limits, cursors, FrontmatterParser(active_limits), SafeMarkdownParser(), DocumentPresenter(),
    )
    history = GitCommitHistory(registry, cursors, active_limits)
    catalogue.discover()
    use_cases = ExplorerUseCases(
        ...
        ListCollection(catalogue, source_browser), GetSettings(catalogue, source_browser),
        ReadDocument(catalogue, source_browser),
    )
