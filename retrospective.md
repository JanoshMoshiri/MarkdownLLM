...
3. **Triage the open conflicts** — walk every `status: open` conflict, the whole set and not only what the floor aged, and force a disposition on each: *rule* (`superseded` / `both-valid` / `dismissed` → `status: resolved`, both parties updated), *link* it from the work that will resolve it, or *hold* it with `disposition: keep-active` + a `disposition_reason` naming what would resolve it. The same shape as item 2, for the same reason: an open conflict is created here (item 4) and at session end, and until 2026-09-08 was read at neither — the framework's higher-stakes object had the weaker lifecycle (`belief-revision.md` → Who Reads An Open Conflict; `circulation-is-not-disposition`).
4. **Surface latent conflicts** — if "What Didn't Work" reveals two positions the domain has been holding simultaneously without acknowledging it, create a `type: conflict` thing
5. **Update specs** if "What Should Change" identifies a concrete improvement to an existing spec or skill
6. **Carry forward open questions as open-loop things** (a `plan` or work thing at a non-terminal status) — the orient view surfaces them next session; there is no continuity brief to update
7. **Commit everything** per the standard `post-write:commit` hard hook
...
3. **Index rebuild** — regenerate the domain's derived indexes from the things and reset their provenance, so the period closes with indexes provably in sync with reality (`validate.thing.md` → Index Integrity). The floor refuses a drifted deployed index at every commit (`mdllm coherence`), so this beat resets the provenance anchors to the period's end rather than restoring parity — parity never depended on it.
4. **Change-driven reconciliation** (`change-reconciliation.md` → Retrospective Reconciliation) — for any change the period made *without* reconciling it at the time, run the full-corpus pass: freeze the period end as a baseline, reconstruct the delta from git, walk the affected set, and seal contradictions via `belief-revision.md`. This is the scan that catches the *twisted* domain — changes that landed with no change-time pass. Surfaced contradictions feed "What Didn't Work"; the realignment feeds "What Should Change." It is what makes running a retrospective *initiate* reconciliation, not merely reflect on it. Its work list is explicit: `mdllm cues` names every cue still `open` and every reasoned-from modification since the last retrospective that no cue covers; this scan answers each — a recorded `verdict` and `verdict_reason` on the cue thing — so the period closes with no question outstanding (`change-reconciliation.md` → The Cue Persists).
...
- **belief-revision.md** — Retrospectives frequently surface latent conflicts that weren't visible at the session level. The retrospective process is a natural entry point for conflict creation — and, since v1.6, the period-cadence reader of the open set: item 3 above triages every open conflict, and scan 6 re-reads the held ones' reasons.
