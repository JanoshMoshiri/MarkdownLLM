...
| `mcp-serve <domain> [--http --port N]` | Serves the domain's exposed face (`exposed: true` things only) over MCP — stdio by default, Streamable HTTP with `--http` (loopback-only; non-loopback binds refused until the OAuth 2.1 leg) — the cross-domain producing side | Wired into a consumer's `.mcp.json` (stdio: `command`; HTTP: `url`); stdio you rarely run by hand, `--http` you run when a porch should outlive its callers |
