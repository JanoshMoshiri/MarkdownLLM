...
| `cues` | `mdllm cues [--since SINCE] [path]` | the cue question, persisted: open `type: cue` things awaiting a human verdict, plus reasoned-from things modified since the last retrospective that no cue covers; session-start emits the same line every session — reports, never raises or answers; exit 0 always |
...
- **`cues`** — When the digest's *Reconciliation cues* line is long and you want the full list, or before a retrospective (it is scan 4's work list).
