...
- **[IN-FLIGHT 2026-06-23] Domain-kernel + harness-hardening build — multi-phase,
  durable-state protocol.** Full plan + progress table: `docs/plans/domain-kernel-hardening.md`
  (the source of truth — read it first on resume). Commits are the ledger; this entry
  ...
  **Next (Phase 3, in progress):** `cmd_session_start` (version-check both legs + velocity,
  prints to stdout for injection) + a `SessionStart` hook in
  `adapters/claude-code.settings.example.json` (currently PostToolUse only) — one
  Claude-format adapter covers Claude Code + VS Code Copilot agent mode; opt-in, interpretation
  stays the floor. Then Phase 4 (deliberate `/end-session` + `/retrospective` slash commands;
  **session-end stays human-decided — no auto catch-up, operator's explicit call**) and
  Phase 5 (rollout via `domain-refresh` + `doctor` staleness + major version bump).
  Targets confirmed: Claude Code + VS Code Copilot; cloud/CLI off-table (interpretation
  floor covers them). PreToolUse security/risk hooks explicitly deferred (future).
