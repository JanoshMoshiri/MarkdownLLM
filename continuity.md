...
- **[COMPLETE 2026-06-23] Domain-kernel + harness-hardening build — all 5 phases
  ...
  **Open follow-ups:** (a) **PreToolUse security/risk hooks** — the agreed next build (operator
  has ideas); the foundation deliberately leaves PreToolUse free. (b) The other live domains
  (eco, property, code-architect) are still on older framework versions and unmigrated to the
  kernel shape — they migrate via the `refresh` rail when next opened (felt-when-felt private
  work). (c) The framework's own root `AGENTS.md` (~23k) was NOT migrated to kernel shape — out
  of scope this pass; candidate if it keeps drifting. (d) Version is **3.15.0 (minor)** — re-tag
  major if the operator prefers.
