...
**The space between sessions gets its first specification, and the docs learn to read the tool.** *First, the record catches up with itself: a push on 20 September carried, under 3.41.0, work that version's entry — written on the 15th — never named. `substrate-native-a2a` was planned and its first three phases built. A `workflow-definition` stage may now declare an `actor` (optional, shape-checked, and deliberately not gate authority — a stage may be executed by an agent and authorised by a human), and **`mdllm watch`** lifts into the floor the doorbell three shell watchers had carried in `case` statements: one remote-ref read per tick, a tree read only when the head moved, the board read from an immutable commit view and never the worktree, two new refusals that turn silent polling into a loud exit, and — found a day later — a per-clone single-instance lock whose Windows liveness probe opens a query-only handle, because `os.kill(pid, 0)` on that platform terminates the process it is asked about; a refusal exits 3, not 0, so a harness bound to re-invoke on exit is never woken for a doorbell that did not ring. Three cues answered, two of them inflections. A live domain armed its three roles against its real corpus; the live turn is still to run, so the scripts stay running until it beats them.* Then a domain agent reported two reviewer clones colliding and proposed a regex on the shell script; the diagnosis was right and the remedy declined, and the operator ruled the real gap its own spec: **`standing-watch.md`** (draft 0.2) — the first specification of a process that runs while no session does, named for the primitive not the command, with the watch contract lifted out of docstrings (remote-ref reads, four arming refusals, exit codes as the wake contract, the five inherited decisions with the failure each prevents), and peers rather than orchestrator and child. Its scope needed a precondition paid first: run membership had been specified as `informed_by` and used as `implements`. The operator ruled **membership is realisation** (`run-membership-is-realisation-2026-09-22`; `workflow-state.md` 0.8 — function, not origin; provenance and membership may diverge), cued as an inflection and walked over seventeen edges. On that edge **`mdllm watch --run <id>`** scopes a watch to one run's members at the definition's stages — two reviewers on two runs in one clone are two watchers — and building it corrected the draft twice: the empty-board guard had silenced the one departure the scope exists to carry, and the pin that "came for free" is the run's own, which only a vertical watch reads. Session-end harvested two insights, one of them a held position rather than a ruling: mesh safety is the floor, not the topology.

**The public docs stop restating what the tool can say.** `docs/operator-guide.md`'s toolbox table and `docs/framework-map.md`'s views restated `mdllm --help` and spec frontmatter by hand, ungated — four to eight hand edits per CLI change. **`mdllm docs`** (the 38th subcommand; `--check` is a pre-commit coherence leg) generates the toolbox from argparse — subcommand, exact usage, the tool's own help, alphabetical — with the authored *when you'd type it* lines kept beneath and checked both ways; the map's Views 2 and 3, which a mermaid diagram cannot host as a managed block, are checked instead (every spec a node whose `(status)` tag matches its frontmatter; every subcommand a node and every node a subcommand) and the map gains a generated list of every declared spec-to-spec edge beside the curated drawing. First run: 33 hand rows against 37 subcommands — six had no row at all — and no View 2 node for `docs/estate-mechanics.md`, invisible to every cold read since August. The full suite's architecture gate then refused the module's one recorded shortcut — a function-local import back into `cli` for the subcommand inventory closes an import cycle — so the seam was inverted rather than the scan evaded: the composition root hands the inventory *down* to the checks, and a call without it says *could not look*, never clean. Two cues answered `inflection`; `AGENTS.md`'s residue sentence walked to match. Phase 2, GitHub Pages from `docs/`, is the operator's switch.

**Two stalls closed by building rather than re-planning.** `coherence-mechanism-build` Phase 3: flow probes 3–5 join the test suite — an invariant breach observable as tree state, refresh end to end, and session close delimiting both the worklog and the verified-flip window — and all three first-run failures were the probe's reading of the contract, not the floor's (zero defects, three misreadings corrected); Phase 4, the operator's cold read, is all that keeps the plan open. `evidence-and-eval-backlog` had said for 56 days that it waited on run-workspace isolation, which a fix on 20 August had already removed while nobody updated the plan; the residue that was real is that a mutated seed left no trace in any result, so the runner now fingerprints the seed tree — bytes, not revisions — before the trials and after each one, records `seed_sha256`/`seed_mutated` on every committed result, and stops the arm on mutation. The eval evening itself waits on the operator's own terminal. Floor: 930 passed, 3 environment-skipped.

## [3.41.0] - 2026-09-15

**The two human acts the membrane waited on, and the birth that no longer chooses for you.** Sixteen days after the watertight-membrane sprint ended `blocked` on the operator — twelve clone-local trust grants and two attributable `verified` flips — both were performed at the operator's seat in one sitting: the grants on their reviewed hashes (`external-trust review` found seven address books and fifteen entries on this clone, three already trusted, the twelve exactly the sprint's staging), and the one flip still owed (the other side had been flipped in an interim session), in its own commit naming him. `estate-check` over the seven consumers: **coverage 271/332, from zero** — and the first thing the lifted blindness showed was the backlog it had hidden: 95 imports needing attention (94 stale, 1 diverged; 23 on the cross-domain overview alone, eleven of them content-changed and owing re-quarantine plus a fresh flip each) and 61 still uncheckable. A watertight membrane is one that *reports* the stale; it now does, and disposition is each consuming domain's under its own contract. The operator queue's Tier 1 is empty. The dispatcher dead-man was answered — the pilot's tick had been deliberately retired, not lost, and its silence left no record, which is the next retrospective's finding — and re-dated for the re-host on the operator's own machine. Then vendor Phase 8, the gate the documentation plan has waited behind since 17 August: ruled and built. **`scaffold` no longer selects a harness when `--harness` is omitted** (`scaffold-harness-is-an-explicit-selection-2026-09-15`): at a keyboard it asks — the registered list, by name or number, aliases accepted — and with nobody there it refuses and prints the choices, so a dispatched run, a CI job, or an agent's shell tool names its choice and reruns. The registry's `selection(None)` raises `HarnessSelectionRequired` instead of defaulting; `DEFAULT_HARNESS` survives only as doctor's subject when no project adapter is present. Twenty test call sites that had leaned on the silent default now name one; six new tests cover the ask, the number, the alias, the re-prompt, the EOF refusal and the headless refusal, the last proven live through the launcher. Eight surfaces restated the old default by hand and were corrected — the per-flag tax `public-docs-face-build` meters, paid once more on the release that unblocks it. The adapter-refresh offer (Phase 8's second box) stands in `domain-refresh.md`; the survey recorded in the vendor plan says thirteen of fourteen domains on this clone carry stale managed fragments, the diff everywhere the launcher probe-guard hardening — adoption is each domain's act at its operator's keyboard, deliberately not a batch. One cue raised and answered by citation; the derived indexes rebuilt. Floor: full suite at 8d51b6f 842 passed, 3 environment-skipped, 4 failed — all four test_runtime call sites still leaning on the old default, made explicit in the release commit and re-run green (40/40 across test_runtime + residual-totality). Validate + coherence clean.

## [3.40.0] - 2026-09-13

**The question that waits, and the agent that may answer it.** Change-reconciliation's cue question had been mechanical since 4 August — the pre-commit `candidates` leg asks *inflection?* of every modified reasoned-from thing — and had nowhere to go: printed to a terminal and gone, and at 3am printed to nobody. The 2026-09-08 review's F5 asked who declares an inflection when a run commits; an external review of 12 September asked, from the other side, where human authority is durably represented; the operator's own words the same day were *make it loud so the user doesn't have to remember*. One ruling answered all three (`unattended-cue-carrier-2026-09-12`): a session may raise a cue, only a human answers, the retrospective is the net. `type: cue` joins the reserved set — `subject` (a structural pointer, validated and reverse-indexed, deliberately *not* counted toward fan-in so a cue cannot raise the odds of the next cue), `raised_at` (a local pin the structural-pin check resolves; a cue raised *in* the modifying commit pins the parent and its birth commit is covered by construction), `raised_by`, and a `verdict: inflection | not-inflection` with a `verdict_reason` the floor requires (Error without both). `mdllm cues` walks the commit stream since the newest retrospective (thirty days when none), keeps modifications by the predicate `candidates` already applies so the two cannot disagree, subtracts what a cue covers, and the session-start digest re-lists every open or unraised cue every session until answered. First live reading on this root: **36** since the 27 August retrospective — the drift the record said was being *felt*, counted. The next morning the operator ruled once more rather than twenty-eight times (`framework-agent-closes-settled-cues-2026-09-13`, extending `settled-reasoning-is-standing-authority`): the framework domain agent answers a cue by citing the decision already on the record (`informed_by`) or on settled reasoning with the walk recorded; every verdict a commit, overturnable by editing; the consequence-permanent rows untouched; dispatch runs still raise and never answer. All 28 traced to four rulings already made — the gates census, the membrane sprint, the Desktop-to-onramp reversal, the conflict-lifecycle review — and were closed by citation in one commit; the commit's own tail closed on itself in one more, which is the fixed point the design needed. *Nothing added, just linked.* Landed alongside: the hook's block messages stop advertising `--no-verify` (every spec said never; the hook said "run with"); trigger-specification 2.1's actions table says who acts and marks `notify` as a seat; the dark-region walk corrected six prose enumerations the registries had outgrown, and four census tests learned the type at the seams they guard. The 2026-09 retrospective ran all seven scans by the F1 wiring for the first time: scan 4's work list mechanically enumerable and empty; one contradiction found *inside* change-reconciliation.md on its second day — the boxed entry rule against the who-answers paragraph — reconciled (1.4); four insights promoted on met conditions (the carrier is `reconciliation-candidates-are-detectable-from-the-commit-stream` built); git-workflow's commit table found ten prefixes behind practice and updated (1.8); the one open conflict held with its proposed ruling for the operator's seat. 842 self-tests (839 passed, 3 environment-skipped at a18600a; 16 new in `test_cues.py`); validate + coherence clean. Examples re-pinned from 3.36.0 (they validate clean under the current floor and use none of what changed). Open: `cue-carrier` Phase 6 waits on a dispatch run raising its first cue; the promoted `an-agent-in-a-loop-optimises-the-loop-not-the-goal` is exposed, so consumers' pins go stale on their next `imports-check`. **After the bump, before the push — two rulings the operator delegated to the framework agent, evidence-led, and one defect the delegation found.** The one open conflict (`hard-hook-vocabulary-contradicts-observable-trigger-insight`, open since 27 August) ruled `both-valid`: the old insight was right about *enforcement* and wrong to bind it to the word "hard"; revised to v1.1 in anchor vocabulary, `orchestration.md` unchanged. And the retrospective's cadence became a dated chase (`retrospective-cadence-is-a-dated-chase-2026-09-13`): the latest retrospective carries a `type: time` trigger thirty days from its `period_end` (the spec's own monthly, the estate's existing form one radius in); running it is gated on activity, not the date; unattended, a dispatch run drafts it and files every ruling to the seat; attended, the mechanical scans run first and the judgement half is offered — never silently run at t=0, because five runs across three harnesses say un-pulled judgement at session start does not happen. retrospective 1.8, dispatch-loop 1.5, the command surfaces, the thing template; no scheduler outside the corpus, no new floor mechanism. The defect: `git log --since=<date>` takes a bare date at the *current time of day*, so on the retrospective's own day the cue walk saw nothing that followed it — fixed to explicit midnight, with a regression test. 842 self-tests measured at a18600a (839 passed, 3 skipped) plus one regression test at the fix; the re-run at the fix reports itself in the commit stream.

## [3.39.0] - 2026-09-10

**A floor that could not see itself reported its own blind spot as a clean bill of health.** On a second machine running git 2.16.1, `session-start` and `doctor` reported `Floor: NOT INSTALLED` against a hook set that was present, byte-current, and executing. The hooks were fine; the question was broken. `git rev-parse --path-format=absolute --git-path hooks` is the resolver behind every hook-path call site; `--path-format` is git ≥ 2.31, and older `rev-parse` does not reject an option it cannot parse — it echoes the option back on stdout and exits 0. Every call site fell back only on a nonzero return code, so the fallback never fired and the echo was consumed as a path — `<root>/--path-format=absolute\n.git/hooks`, which cannot exist on any filesystem. Two of the three consequences were loud but harmless: a false `NOT INSTALLED` at every session start, and a `DEGRADED` verdict advising hand-validation the floor was already performing. The third was neither. `repository_transaction._run_hook` failed `is_file()` on the impossible path and returned down its "Git commit also treats an absent hook as success" branch, **silently skipping the validation hook inside a scaffold transaction** — so `mdllm scaffold` on a pre-2.31 git would have created a domain with validation unrun and reported success. Ordinary commits were never exposed, because git invokes `.git/hooks` itself; only mdllm's own transactional hook-invocation path was. One owner for the fix: `hook_contract.rev_parse_path` discards output whose first line is the option itself, letting each call site's already-written unsupported-git fallback do the job it was written to do, applied at all three live call sites. Two tests had been green *over* the defect and are corrected — one built its expected hooks path from the same raw `--path-format` output the resolver used, mutually consistent garbage; the other keyed "UNTESTED is acceptable" to `git hook run` support, when `run_git_hook`'s old-git `sh` route makes a pre-2.36 git testable too. Verified on git 2.16.1: `doctor` DEGRADED → FLOOR ACTIVE, and session-start emits no Floor line at all. Three tests in `test_repository_transactions.py` still fail on a pre-2.36 git because they invoke `git hook run` and `git worktree add -q` in the test body and need the newer git in the harness itself — environment, not shipped code, and left failing rather than weakened.

## [3.38.1] - 2026-09-09

**A short commit pin that YAML types as a number keeps its spelling.** The 3.38.0 push failed its Linux CI leg on `test_imports_freshness_over_http_with_token`: a healthy import read `stale`. The cause was neither the porch nor the release — the test's `git rev-parse --short` pin drew a spelling YAML 1.1 types as an integer (a leading zero plus octal digits, `0123456` → 42798; or `0b` plus binary), and `str()` of that value is a number the source never held. The v3.27.0 guard in `_pins_match` had cured only the decimal case, which `str()` reproduces; measured through the project's own loader, the octal/binary draw is one pin in 1069 — and at `0000000` the same draw types as int 0, falsy, so a fully-declared import was filed "incomplete". Fixed at the seam that already existed for the same disease: `LexicalInt` beside `LexicalFloat` in the strict loader retains the YAML token (registered for SafeDumper; booleans untouched under their own tag), `structural_refs.scalar_lexeme` reads it back, and `iter_commit_pins`, `_pins_match` and `imports_freshness` compare and test presence on the text rather than the number. Deterministic regressions cover every former failure spelling; no test draws a random hash to prove it.

## [3.38.0] - 2026-09-09

**A product direction was set, tested against a real person, and reversed in nine days — with both records kept.** `markdownllm-desktop-is-primary-accessible-product` had made the first-party Desktop app the accessible path; three days of telemetry, one onboarding session with a non-developer operator, and one diagnosis reversed it. `harness-native-onramp-supersedes-desktop` supersedes it — Desktop freezes at 0.1.6, and `interface.md` v1.7 replaces the first-party Desktop route with a harness-native onramp while the substrate stays route-agnostic and each route's claim is earned by its own evidence. The superseded decision's record stands and its requirements are retained as onramp input: this is a supersession, not a deletion. Three insights carry the reasoning — `the-onramp-is-the-operators-own-sentence` (ninety minutes of already-mechanised steps that landed nothing, and the sentence the person reached for instead), `a-surface-without-a-floor-accumulates-repairs-not-progress` (the diagnosis the freeze applies), and `the-record-answers-the-direction-question-first` (how the decision's Context was reached: telemetry, the prior record's own repair list, and the fired triggers answered before judgement did). The Explorer white-label presentation increment was specified to v0.6, independently cold-reviewed, then parked with its requirements deferred; the README front page was reconciled to follow the decision, and a design cold review that was stopped before producing a record is recorded as stopped rather than quietly dropped.

**Explorer reached 0.4.3 across both platforms.** A portable macOS 0.4.0 candidate shipped and the Windows 0.4.0 installer was built and verified; macOS file reads were repaired along with a fault that stopped Explorer staying up. The side panels were matched, the packaged brand icon exposed, and the Desktop theme installed and aligned through 0.4.2. A Mac quick start and a documentation map landed, and the implementation, documentation and coordination handoffs were each closed on the record rather than trailing off.

**The dispatcher learned what to do when the floor refuses it, and the open conflict finally got a reader.** `mdllm dispatch-payload` moves launch-text composition into the corpus so the scheduler only carries it; a run repairs to convergence, states its own digest, lands that digest in the worked repo rather than the framework root the host cannot push, and arms a dead-man with its limit stated — an invalid launch prints its digest instead of failing silently, and the advisory claim's lease is sized against the tick. Closing the release, an external review read the two ritual command files as the actual entry points and found the retrospective firing three of its spec's seven scans: change-driven reconciliation — the backward pass both `retrospective.md` and `change-reconciliation.md` bind to that hook, and the only one that catches an inflection nobody declared — was never wired to the hand that casts it. All four command surfaces now fire all seven, plus whole-set conflict triage. `belief-revision.md` v1.3 gives an open conflict the two readers an insight already had (the session-end brake and the retrospective triage, on the same `disposition: keep-active` marker) and defines what a consumer does while one is open — proceed and flag, never synthesise a middle position, never pick silently. The floor gained `conflict_age_findings`, the 30-day stale-conflict Info both specs had promised since v1.0 and neither had carried; it fires *through* a live inbound edge by design, because an edge keeps a conflict in circulation and only a disposition rules on it. Framework and both example corpora validate clean; the suite is 815 passed, 3 skipped, with one pre-existing Windows race in a concurrency test's own instrumentation — reproduced at the prior commit and left for its own fix.
