...
> **Codex desktop / Windows managed-shell adapter (verified 2026-08-11):** this adapter was added after the framework ran in the Codex desktop harness, where `python` was absent from PATH and the bundled Python lacked PyYAML. If `python` is unavailable, run `./tools/mdllm.ps1 <command>` instead. It prefers the repository's gitignored `.venv` and then falls back to a discoverable Python; this is command-equivalent to `python tools/mdllm.py <command>`. It is measured Codex-harness evidence, not a claim that every managed shell has the same runtime shape.
