...
3. If adding or removing a spec, run `mdllm coherence` — **run it to see what it covers; the module is the authority and this line is not a list of its checks** (a prose inventory of mechanical checks is the exact restatement class the checks exist to end, and it went stale inside one sprint). It runs in the pre-commit hook, so a stale generated artifact, a catalog annotation disagreeing with its spec's frontmatter, or a Tier-2 spec no routing row names all block the commit. Then walk the **prose-only residue** the tool still cannot read (see `change-reconciliation.md` → Walking the Dark Region): today that is `docs/framework-map.md` (View 1 counts, the drawn edges — which spec a node serves — and the `accDescr` prose; the nodes themselves and their `(status)` tags are mechanical since `mdllm docs`), the *descriptions* in this file's routing table and spec catalog, and the authored *when* lines under the toolbox in `docs/operator-guide.md` — their annotations and their completeness are now mechanical, and only the human-written one-liners remain judgement.
...
- **belief-revision.md** — How the framework handles contradictions between things: `type: conflict`, `relation: supersedes`/`contradicts`, the belief revision process, who reads an open conflict (the session-end brake and retrospective triage, on the floor's orphan and 30-day age findings), and the default for reasoning while one is open. (`type: specification`, `status: stable`)
...
- **change-reconciliation.md** — How a domain stays consistent across change: the human declares an inflection, then a scale-free four-beat pass (cue → assimilate → walk → seal) reconciles the change against its blast radius using the relationships and reverse-provenance indexes. Semantic consistency is maintained at the point of change, not by sweeping. The cue question also *persists* when nobody answers it at the boundary — `type: cue` carries the question and a human `verdict`, and the session-start digest re-lists every open or unraised one until it is answered (any session may raise; a human answers, or the framework agent by citing the decision that already ruled it; an unattended run never does). (`type: specification`, `status: draft`)
...
- **standing-watch.md** — The first specification of the between-sessions surface: a process that runs while no session does, observes committed state on the remote ref, and wakes an agent when a declared condition turns true there. Owns the `mdllm watch` contract — remote-ref reads never the worktree, the five inherited decisions, arming refusals, exit codes as the harness wake contract — and specifies the scope of a watch as a `workflow-run`, not built until `workflow-state.md` settles the membership edge. Peers, not orchestrator and child. (`type: specification`, `status: draft`)
