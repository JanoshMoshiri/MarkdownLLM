...
> **Windows / managed-shell adapter:** if `python` is not on PATH, run `./tools/mdllm.ps1 <command>` instead. It prefers the repository's gitignored `.venv` and then falls back to a discoverable Python; this is command-equivalent to `python tools/mdllm.py <command>`.
